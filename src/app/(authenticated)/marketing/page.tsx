...
import LoadingComponent from "@fck/components/loading";
import { CreatePost } from "@fck/components/create-post";
import { Button } from "@fck/components/ui/button";
import { ArrowRight, CheckCircle, BotMessageSquare, Database, Zap } from "lucide-react";
...
const features = [
  {
    icon: Zap,
    title: "Next.js 14+",
    description: "Built with the latest App Router for optimal performance and developer experience.",
  },
  {
  ...
  },
  {
    icon: Database,
    title: "tRPC & Prisma",
    description: "End-to-end typesafe APIs connecting directly to your database.",
  },
  {
    icon: CheckCircle,
    title: "Stripe Ready",
    description: "Integrate payments and subscriptions easily (code structure included).",
  },
];
    ...
    <main className="flex min-h-screen w-full flex-col items-center bg-background text-foreground">
      <section className="w-full bg-gradient-to-b from-background via-background to-muted/30 py-24 text-center md:py-32 lg:py-40">
        <div className="container mx-auto max-w-4xl px-5">
          ...
          >
            <Balancer>
              A Next.js boilerplate with Clerk, tRPC, Prisma, Stripe, and more. Stop rebuilding auth and basic setup, start building your features.
            </Balancer>
          </Animated_p>
          <Animated_div
           ...
           >
             {user ? (
               <>
                 ...
                 <Button asChild size="lg">
                   <Link href="/app/post">Go to App posts <ArrowRight className="ml-2 h-5 w-5" /></Link>
                 </Button>
                 <Button asChild size="lg">
                   <Link href="/tos">Terms of Service <ArrowRight className="ml-2 h-5 w-5" /></Link>
                 </Button>
                 <Button asChild size="lg">
                   <Link href="/privacy">Privacy Policy <ArrowRight className="ml-2 h-5 w-5" /></Link>
                 </Button>
                 <Button asChild size="lg">
                   <Link href="/">Home <ArrowRight className="ml-2 h-5 w-5" /></Link>
                 </Button>
                 <Button asChild size="lg">
                   <Link href="/docs">Docs <ArrowRight className="ml-2 h-5 w-5" /></Link>
                 </Button>
                 <SignOutButton>
         ...
         <div className="container mx-auto max-w-5xl px-5">
            ...
            <div className="mt-12 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
               {features.map((feature) => (
                  <div key={feature.title} className="flex flex-col items-center rounded-lg border border-border bg-card p-6 text-center shadow-sm transition-shadow hover:shadow-md">
